...
//checkin user is logged in or not
window.onload = () =>{
    if(user){
...
const validateForm =() =>{
    if(!productName.value.length){
        ...
        return showAlert('short description must be beetween 10 to 100 letters long');
    } else if(!des.value.length){
        return showAlert('enter detail desciption about the product')
    ...
    } else if(!actualPrice.value.length || !discount.value.length ||
...
addProductBtn.addEventListener('click' , () =>{
    storeSizes();
    // validate form
    if(validateForm()){// validateForm return true or false wjhile doing validation
