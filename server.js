...
let serviceAccount = require("./ecom-website-30c91-firebase-adminsdk-s9mkv-da32a4f97b.json");
...
const bucketName = "ecom-website-turial";
...
app.post('/signup', (req,res) => {
    ...
    //form validations
    if(name.length < 3){
        return res.json({'alert': 'name must ve 3 letters long'})
    }else if(!email.length){
       return res.json({'alert' : 'enter your email'});
    }else if(password.length <8){
        return res.json({'alert' :'password should ve 8 letters long'});
    }else if(!number.length){
        return res.json({'alert':'enter your phone number'});
    }else if(!Number(number) || number.length < 10){
        return res.json({'alert':'Invalid number, please enter your valid one'});
    }else if(!tac){
    ...
    .then(user => {
        if(user.exists){
            ...
            // encrypt the password before stpring it
...
app.post('/login', (req, res) => {
    ...
    db.collection('users').doc(email).get()
    .then(user => { 
        if(!user.exists){// is email do not exitst
...
app.post('/seller', (req, res) =>{
    ...
    if(!name.length || !address.length || number.length<10 ||
        !about.length || !Number(number) ){
            return res.json({'alert' : 'some information(s) is/are invalid'});
        }else if(!tac || !legitInfo){
            return res.json({'alert' : 'you must agree to our terms and conditons'})
        }else{
...
app.post('/add-product', (req, res) => {
    ...
    if(!draft){
        if(!name.length){
            ...
            return res.json({'alert' :'short description must be beetween 10 to 100 letters long'});
        } else if(!des.length){
            return res.json({'alert' :'enter detail desciption about the product'});
        ...
        } else if(!actualPrice.length || !discount.length ||
            !sellPrice.length){
...
app.post('/order', (req, res) => {
    ...
    const mailOption = {
        from: 'Trancongminh98xm@gmail.com',
        to: email,
...
<body>
    ...
    let docName = email + Math.floor(Math.random() * 1234567895445);
